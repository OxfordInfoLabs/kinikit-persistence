...
class SQLite3DatabaseConnectionTest extends \PHPUnit\Framework\TestCase {
    ...
    public function testCanParseFunctionRemappings() {
        ...
        $this->assertEquals("SUM(COUNT(test)) OVER ()", $sqlite3Connection->parseSQL($sql));


        $sql = "SUM_TOTAL(test)";
        $this->assertEquals("SUM(SUM(test)) OVER ()", $sqlite3Connection->parseSQL($sql));


        $sql = "COUNT_PERCENT(test)";
        $this->assertEquals("100 * COUNT(test) / SUM(COUNT(test)) OVER ()", $sqlite3Connection->parseSQL($sql));


        $sql = "SUM_PERCENT(test)";
        $this->assertEquals("100 * SUM(test) / SUM(SUM(test)) OVER ()", $sqlite3Connection->parseSQL($sql));

        $sql = "ROW_NUMBER()";
        $this->assertEquals("ROW_NUMBER() OVER (ORDER BY 1=1,1=1)", $sqlite3Connection->parseSQL($sql));
