...
class TableQueryEngineTest extends \PHPUnit\Framework\TestCase {
    ...
    public function testCanQueryForRowsUsingDefaultConnection() {
        ...
        $queryEngine = new TableQueryEngine("example");
        $this->assertEquals([
            1 => ["id" => 1, "name" => "Mark", "last_modified" => "2010-01-01"]
        ], $queryEngine->query("WHERE name = ?", ["Mark"]));


        $this->assertEquals([
            1 => ["id" => 1, "name" => "Mark", "last_modified" => "2010-01-01"],
            3 => ["id" => 3, "name" => "Dave", "last_modified" => "2014-01-01"]
        ], $queryEngine->query("WHERE name = ? or name = ? ORDER by id", ["Mark", "Dave"]));
    ...
    public function testIfManyToOneRelationshipIsDefinedQueriesAlsoQueryRelatedEntitiesViaJoin() {
        ...
        $this->assertEquals([1 => ["id" => 1, "name" => "Mary Jones", "child_id" => null]], $queryEngine->query("WHERE id = 1"));
        $this->assertEquals([2 => ["id" => 2, "name" => "Jane Walsh", "child_id" => 1, "child1" =>
            ["id" => 1, "description" => "Washing", "child2_id" => null]]], $queryEngine->query("WHERE id = 2"));

        $this->assertEquals([3 => ["id" => 3, "name" => "James Smith", "child_id" => 2, "child1" =>
            ["id" => 2, "description" => "Swimming", "child2_id" => null]]], $queryEngine->query("WHERE id = 3"));


        // Now do a regular query
        $results = $queryEngine->query("WHERE name LIKE ? OR child_id = ?", "JA%", 1);
        ...
        $this->assertEquals([4 => ["id" => 4, "name" => "Heather Wright", "child_id" => 3, "child1" =>
            ["id" => 3, "description" => "Cooking", "child2_id" => 1, "child2" => [
                "id" => 1, "profession" => "Doctor"]
            ]]], $queryEngine->query("WHERE child1.description = 'Cooking'"));


        $this->assertEquals([4 => ["id" => 4, "name" => "Heather Wright", "child_id" => 3, "child1" =>
            ["id" => 3, "description" => "Cooking", "child2_id" => 1, "child2" => [
                "id" => 1, "profession" => "Doctor"]
            ]]], $queryEngine->query("WHERE child1.child2.id = 1"));
    ...
    public function testIfOneToOneRelationshipDefinedQueriesAlsoQueryRelatedEntities() {
        ...
        $this->assertEquals([["id" => 1, "name" => "Mary Jones", "child_id" => null,
            "child1" => ["id" => 2, "description" => "Swimming", "parent_id" => 1]]], array_values($queryEngine->query("WHERE id = 1")));


        $this->assertEquals([["id" => 2, "name" => "Jane Walsh", "child_id" => 1,
            "child1" => ["id" => 3, "description" => "Cooking", "parent_id" => 2]]], array_values($queryEngine->query("WHERE id = 2")));


        // Try a filter query.
        $results = $queryEngine->query("WHERE name LIKE ?", "JA%");
        ...
        ]], array_values($queryEngine->query("WHERE child1.description = 'Walking'")));
    ...
    public function testIfOneToManyRelationshipDefinedQueriesAlsoQueryRelatedEntities() {
        ...
        $this->assertEquals([["id" => 3, "name" => "James Smith", "child_id" => 2,
            "child1" => [["id" => 4, "description" => "Laughing", "parent_id" => 3],
                ...
                ["id" => 6, "description" => "Walking", "parent_id" => 3]]]], array_values($queryEngine->query("WHERE id = 3")));
        ...
        $this->assertEquals(4, sizeof($queryEngine->query("LIMIT 4")));
    ...
    public function testIfManyToManyRelationshipDefinedQueriesAlsoQueryRelatedEntities() {
        ...
        ], array_values($queryEngine->query("WHERE id = 1")));


        $results = $queryEngine->query("WHERE manytomany.profession = ?", "Car Mechanic");
        $this->assertEquals(2, sizeof($results));
        $this->assertEquals($queryEngine->query("WHERE id IN (2, 3)"), $results);
